...
import { resolvedSession, todayStr, isBefore, addDays, diffDays } from '../lib/scheduler';
...
export default function Stats({ completedDays, programStart, overrides }) {
  ...
  let checkDate = today;
  while (true) {
    const s = resolvedSession(checkDate, programStart, overrides);
    if (!s) break;
    if (s === 'rest' || completedDays[checkDate]?.allDone) {
      if (s !== 'rest') streak++;
  ...
  const stats = [
    ...
    { label: 'STREAK',   value: streak, suffix: 'days', color: 'var(--primary)' },
